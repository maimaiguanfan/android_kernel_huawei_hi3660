		...
		unsigned int as_nr)
{
	...
	if (max_loops == 0) {
		...
		if (dsm_report_enable(kbdev->error_num.as_bit_stuck, 20, kbdev->runmode_normal)) {
			gpu_dsm_report(kbdev, GPU_BIT_STUCK_DSM_NO, "AS_ACTIVE bit stuck.");
		}
		kbdev->error_num.as_bit_stuck++;
#endif
